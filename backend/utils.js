...
const filterAndFormatData = (currencyData, allCurrencies) => {
  ...
  const targetCurrencies = allCurrencies.filter(
...
const reshapeTargetPrices = (targetPrice, targetCurrencies) => {
  const filteredTargetPrices = targetPrice
    .filter((targetData) => targetCurrencies.includes(targetData.currencyName))
    .map((targetData) => {
...
const formatExchangeRateData = (
  currencyData,
  baseCurrencies,
  targetCurrencies
) => {
  const baseCurrencyData = currencyData
    .filter((data) => baseCurrencies.includes(data.baseCurrency))
    .map((data) => {
      const { baseCurrency, targetCurrencies: targetPrice } = data

      const formattedTargetPrices = reshapeTargetPrices(
        targetPrice,
        targetCurrencies
      )
      return {
...
const formatHistorialData = (priceAtTime, targetCurrency) => {
  ...
  const value = targetCurrencies.find(
    (currencyData) => currencyData.currencyName === targetCurrency
  ).currencyPrice
...
export { filterAndFormatData, formatExchangeRateData, formatHistorialData }
